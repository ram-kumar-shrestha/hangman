...
import { FormGroup, FormLayout, Info, Layout } from "../../components";
import { RegisterUserValidate } from "../../helpers";
import { useLocalStorage } from "../../hooks";
import { getAllDropdownOutlets, registerUser } from "../../store/action";
...
const RegisterUser = ({
  ...
  useEffect(() => {
    document.title = "Syanko Katti Roll - Add an User";
  ...
  const formik = useFormik({
    initialValues: {
      ...
      // default value for when outlet owner register
      OutletId: role === "OutletOwner" ? outletId : dropdown[0]?.outletId,
      UserRole: role === "OutletOwner" ? "OutletEmployee" : "OutletOwner",
      password: "",
    },
      ...
      registerUser(JSON.stringify(values))
        ...
        .catch((e) => {
          switch (e.data.status) {
            case "SameUserNameException":
              setMessage({
                desc: e.data.message,
                type: "danger",
              });
              break;
            case "UserNameException":
              setMessage({
                desc: e.data.message,
                type: "danger",
              });
  ...
  return (
    <>
      <FormLayout formik={formik} title="Add a User" svg={add_owner} type="add">
        {/* owner add info */}
        <Info info={message} />
        ...
        {role !== "OutletOwner" ? (
          <>
            <FormGroup
              ...
              error={formik.errors.OutletId}
              touched={formik.touched.OutletId}
              options={dropdown.map(({ outletId, name }) => {
                return { id: outletId, name };
              })}
            />
            ...
            <FormGroup
              ...
              options={[
                { id: "OutletOwner", name: "OutletOwner" },
                { id: "OutletEmployee", name: "OutletEmployee" },
              ]}
            />
          </>
        ) : null}
