...
import { EditUserValidate } from "../../helpers";
import { useLocalStorage } from "../../hooks";
import { getAllOutlets, getSingleUser, editUser } from "../../store/action";
...
const EditUser = ({ outlet, user, getAllOutlets, getSingleUser, editUser }) => {
  const [message, setMessage] = useState({});
  const [backButton, setBackButton] = useState("");
  // const [mappedOutlet, setMappedOutlet] = useState([])
  ...
  useEffect(() => {
    ...
    // .then(() => {
    //   setMappedOutlet(
    //     outlet.map(({outletId, name}) => {
    //       return {name, ['id']: outletId}
    //     }),
    //   )
    // })
  ...
  const formik = useFormik({
    initialValues: {
      ...
      OutletId: user.outletId ? user.outletId : "",
      // UserRole: user.userRole ? user.userRole : '',
    },
    enableReinitialize: true,
    validate: EditUserValidate,

    onSubmit: (values) => {
      editUser(id, JSON.stringify(values))
        .then(() => {
          setBackButton(
            <Link
              to="../view-all-users"
  ...
  return (
    <>
      ...
      >
        ...
        {/*  
        {role != 'OutletOwner' &&
        (user.userRole === 'OutletOwner' ||
          user.userRole === 'OutletEmployee') ? (
          <FormGroup
            inputMethod="select"
            id="OutletId"
            label="Outlet:"
