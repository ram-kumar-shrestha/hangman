...
import { getAllDropdownOwners, addOutlet } from "../../store/action";
...
const AddOutlet = ({ dropdown, addOutlet, getAllDropdownOwners }) => {
  ...
  useEffect(() => {
    document.title = "Syanko Katti Roll - Add an Outlet";

    getAllDropdownOwners()
      .then(() => {})
      .catch((e) => console.log(e));

    console.log(dropdown[0]?.ownerDetailId);
  }, [dropdown.length]);
  ...
  const formik = useFormik({
    initialValues: {
      ...
      IsActive: true,
      IsCustomPrice: false,
      OwnerDetailId: dropdown[0]?.ownerDetailId,
      OrderLimit: "",
    },
    ...
    onSubmit: (values) => {
      // console.log(values);
      addOutlet(
        JSON.stringify({ CentralKitchenId: centralKitchenId, ...values })
      )
        .then(() => {
          setBackButton(
            <Link
          ...
          setMessage({
            desc: "Outlet added successfully.",
            type: "success",
          });
        })
        .catch((e) => {
    ...
    <>
      ...
      >
        ...
        {/* <FormGroup
          ...
          error={formik.errors.IsCustom}
          touched={formik.touched.IsCustom}
        /> */}
      </FormLayout>
    </>
  );
...
export default connect(mapStateToProps, { getAllDropdownOwners, addOutlet })(
  AddOutlet
);
