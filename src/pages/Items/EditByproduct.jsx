...
import { getSingleByproduct, editByproduct } from "../../store/action";
...
const EditByproduct = ({ item, getSingleByproduct, editByproduct }) => {
  const [message, setMessage] = useState({});
  const [backButton, setBackButton] = useState("");
  const { id } = useParams();

  useEffect(() => {
    document.title = "Syanko Katti Roll - Edit a Byproduct";

    getSingleByproduct(id);
  }, [id]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      editByproduct(id, JSON.stringify(values))
        .then(() => {
          setBackButton(
            <Link
              to="../by-products"
          ...
          setMessage({
            desc: "Order edited successfully",
            type: "success",
          });
        })
        .catch((e) => {
...
const mapStateToProps = (state) => {
  return {
    item: state.singleByProductItem,
  };
};
export default connect(mapStateToProps, { getSingleByproduct, editByproduct })(
  EditByproduct
);
